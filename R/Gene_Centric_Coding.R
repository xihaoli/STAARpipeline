...
#' @return A list of data frames containing the STAAR p-values (including STAAR-O or STAAR-B in imbalanced case-control setting), or AI-STAAR p-values under ancestry-informed analysis, corresponding to each coding functional category of the given gene.
#' If \code{find_weight} is TRUE, returns a list containing the AI-STAAR p-values corresponding to each coding functional category of the given gene, as well as the ensemble weights under two sampling scenarios 
...
Gene_Centric_Coding <- function(chr,gene_name,category=c("all_categories","plof","plof_ds","missense","disruptive_missense","synonymous","ptv","ptv_ds","all_categories_incl_ptv"),
	...
	if(category=="all_categories")
	{
		results <- coding(chr,gene_name,genofile,obj_nullmodel,genes,
		                  ...
		                  SPA_p_filter=SPA_p_filter,p_filter_cutoff=p_filter_cutoff,use_ancestry_informed=use_ancestry_informed,
		                  find_weight=find_weight,silent=silent)
	}
